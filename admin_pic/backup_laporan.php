...
// jangan percaya begitu saja pilihan dari GET/form tersembunyi). Selalu dicek
// terhadap $cabang_ids supaya PIC tidak bisa input untuk cabang yang bukan
// tanggung jawabnya, walau id_cabang di-utak-atik manual di form/URL.
// -----------------------------------------------------------------------------
function ambil_cabang_valid(mysqli $conn, int $id_cabang, array $cabang_ids): ?array
{
    if ($id_cabang <= 0 || !in_array($id_cabang, $cabang_ids, true)) return null;
...
if (isset($_POST['tandai_libur'], $_POST['id_cabang'], $_POST['tanggal'])) {
    ...
    if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $p_tanggal) || $p_tanggal > date('Y-m-d') || !ambil_cabang_valid($conn, $p_id_cabang, $cabang_ids)) {
    ...
    if ($status_now === 'lengkap') {
        echo "<script>alert('Laporan tanggal ini sudah diproses, tidak bisa ditandai libur.'); history.back();</script>";
        exit;
    }
    ...
    $stmt->execute();
    audit($conn, 'tandai_libur_oleh_pic', 'laporan_cabang', $p_id_cabang . '@' . $p_tanggal, [
...
if (isset($_POST['batalkan_libur'], $_POST['id_cabang'], $_POST['tanggal'])) {
    ...
    $p_id_cabang = (int) $_POST['id_cabang'];
    $p_tanggal   = $_POST['tanggal'];
    if (preg_match('/^\d{4}-\d{2}-\d{2}$/', $p_tanggal) && ambil_cabang_valid($conn, $p_id_cabang, $cabang_ids)) {
        $stmt = $conn->prepare("DELETE FROM laporan_cabang WHERE id_cabang = ? AND tanggal = ? AND status_laporan = 'libur'");
        $stmt->bind_param("is", $p_id_cabang, $p_tanggal);
        $stmt->execute();
        audit($conn, 'batalkan_libur_oleh_pic', 'laporan_cabang', $p_id_cabang . '@' . $p_tanggal, [
            'id_cabang' => $p_id_cabang, 'tanggal' => $p_tanggal,
        ]);
    }
    echo "<script>window.location.replace('backup_laporan.php?tanggal=$p_tanggal&id_cabang=$p_id_cabang');</script>";
...
if (isset($_POST['simpan'], $_POST['id_cabang'], $_POST['tanggal'])) {
    ...
    $p_id_cabang = (int) $_POST['id_cabang'];
    $p_tanggal   = $_POST['tanggal'];
    $cabang_valid = ambil_cabang_valid($conn, $p_id_cabang, $cabang_ids);
    ...
    if ($stmt->execute()) {
        audit($conn, 'nota_kirim_oleh_pic', 'laporan_cabang', $p_id_cabang . '@' . $p_tanggal, [
            'id_cabang' => $p_id_cabang, 'tanggal' => $p_tanggal,
        ]);

        // Notifikasi ke PIC LAIN yang juga memegang cabang ini (kalau ada) — bukan ke
        // diri sendiri, karena yang mengirim nota ini justru PIC yang sedang login.
        $pic_lain = array_values(array_diff(pic_untuk_cabang($conn, $p_id_cabang), [$id_user_nota]));
        kirim_notifikasi($conn, $pic_lain, 'nota_masuk',
            'Nota Baru: ' . $cabang_valid['nama_cabang'],
            'Nota untuk cabang ' . $cabang_valid['nama_cabang'] . ' tanggal ' . date('d M Y', strtotime($p_tanggal)) . ' diinput oleh rekan PIC.',
            'input_laporan.php?id_cabang=' . $p_id_cabang . '&tanggal=' . $p_tanggal
        );

        echo "<script>
                alert('Nota berhasil disimpan atas nama cabang " . addslashes($cabang_valid['nama_cabang']) . ".');
...
if (!empty($cabang_ids)) {
    ...
    $stmt = $conn->prepare("SELECT COUNT(*) total FROM cabang c WHERE c.id_cabang IN ($placeholders) $where_cari");
    $count_types  = str_repeat('i', count($cabang_ids)) . $cari_types;
    $count_params = array_merge($cabang_ids, $cari_params);
    ...
    $sql = "SELECT c.id_cabang, c.nama_cabang, lc.status_laporan, lc.foto_nota1, lc.foto_nota2, lc.foto_nota3, lc.foto_nota4
            ...
            WHERE c.id_cabang IN ($placeholders) $where_cari
            ORDER BY c.nama_cabang ASC
            LIMIT ? OFFSET ?";
    $stmt = $conn->prepare($sql);
    $params_list = array_merge([$tanggal], $cabang_ids, $cari_params, [$limit, $offset]);
    $types_list  = 's' . str_repeat('i', count($cabang_ids)) . $cari_types . 'ii';
...
// -----------------------------------------------------------------------------
$cabang_terpilih = $id_cabang_pilih ? ambil_cabang_valid($conn, $id_cabang_pilih, $cabang_ids) : null;
...
<div class="container-fluid py-3 px-md-4">
    ...
    <div class="alert alert-info rounded-4 mb-4">
        ...
        Kalau ada cabang yang Anda pegang lupa kirim nota harian, Anda bisa mengirimkan nota atas nama cabang tersebut di sini.
        Laporan tetap masuk ke antrian Anda seperti biasa untuk diproses.
    </div>
    ...
    <div class="badge-info-cabang d-flex align-items-center mb-4 p-3 rounded-4" style="background: linear-gradient(135deg, #1e3a5f 0%, #12233a 100%); color:#fff;">
    ...
    <?php if ($status_terpilih === 'lengkap'): ?>
        <div class="alert alert-success d-flex align-items-center rounded-4 mb-4">
            ...
            <div>
                <strong>Laporan tanggal <?= date('d M Y', strtotime($tanggal)) ?> sudah diproses.</strong>
    ...
    <?php elseif ($status_terpilih === 'menunggu'): ?>
        <div class="alert alert-warning d-flex align-items-center rounded-4 mb-4">
            ...
            <div>Nota tanggal <?= date('d M Y', strtotime($tanggal)) ?> sudah terkirim, menunggu diproses.</div>
        </div>
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <div class="card card-custom mb-4">
            ...
            <div class="card-body p-4 p-mobile-custom">
                ...
                <div class="mt-3">
                    <label class="form-label"><i class="bi bi-chat-left-text me-1"></i> Catatan (opsional)</label>
                    <textarea name="keterangan_nota" class="form-control form-control-custom" rows="3" placeholder="Mis. diinput karena cabang belum sempat kirim nota."><?= h($laporan_terpilih['keterangan_nota'] ?? '') ?></textarea>
                </div>
            </div>
        </div>
