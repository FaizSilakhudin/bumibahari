...
$cabang_ids = pic_cabang_ids($conn, current_user_id());
if (empty($cabang_ids)) {
    http_response_code(403);
    exit('Anda belum ditugaskan ke cabang manapun.');
}

// ==========================================================
// FILTER — dibatasi hanya cabang yang dipegang PIC ini.
// ==========================================================
$tgl_awal  = $_GET['tgl_awal'] ?? date('Y-m-01');
$tgl_akhir = $_GET['tgl_akhir'] ?? date('Y-m-d');
$id_cabang = $_GET['id_cabang'] ?? '';
if ($id_cabang !== '' && !in_array((int) $id_cabang, $cabang_ids, true)) {
    $id_cabang = '';
}
...
$where_sql = "WHERE l.tanggal BETWEEN ? AND ? AND l.id_cabang IN ($ph)";
$params = array_merge([$tgl_awal, $tgl_akhir], $cabang_ids);
$types = "ss" . $types_ids;
