...
| `klaim_bulanan` | Baris klaim manual ("10. Klaim Bulanan" di menu Rekapitulasi) — daftar dinamis (ditambah/dihapus bebas oleh pusat/PIC), bukan daftar tetap. Nominalnya otomatis mengurangi Net Profit SEBELUM admin fee 3% dipotong. `sumber_dana` ('investor'/'warung', default 'warung') — kalau 'investor', nominalnya JUGA otomatis masuk ke "Pengembalian Dana Talangan" di Koreksi Dividen: Sisi Investor (menambah Total Bersih Investor); kalau 'warung', cuma memotong Net Profit tanpa efek tambahan. `urutan_pengelola` sama fungsinya seperti di `revenue_sharing`. |
