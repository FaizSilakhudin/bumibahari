...
| `klaim_bulanan` | Baris klaim manual ("10. Klaim Bulanan" di menu Rekapitulasi) — daftar dinamis (ditambah/dihapus bebas oleh pusat/PIC), bukan daftar tetap. Nominalnya (apapun sumber_dana-nya) otomatis mengurangi Net Profit SEBELUM admin fee 3% dipotong. `sumber_dana` ('investor'/'warung'/'pusat', default 'warung'): 'investor' JUGA otomatis masuk ke "Pengembalian Dana Talangan" (Koreksi Dividen: Sisi Investor, menambah Total Bersih Investor); 'pusat' JUGA otomatis masuk ke "Admin Management Pusat" (6. Rekapan Hasil Akhir Keuntungan); 'warung' cuma memotong Net Profit tanpa efek tambahan. `urutan_pengelola` sama fungsinya seperti di `revenue_sharing`. |
