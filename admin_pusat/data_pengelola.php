...
if (isset($_POST['edit'])) {
    ...
    if ($ada_perubahan) {
        ...
        $insert_stmt = $conn->prepare("INSERT INTO `users` (`username`, `password`, `nama_pengelola`, `no_rekening`, `nama_bank`, `atas_nama_rekening`, `tgl_mulai`, `tgl_selesai`, `role`, `id_cabang`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $insert_stmt->bind_param("sssssssssi", $username, $pass, $nama, $no_rek, $bank, $atas_nama, $tgl_mulai, $tgl_selesai, $role, $id_cabang);
        $insert_stmt->execute();
        $insert_stmt->close();
            ...
            <table class="table table-saas align-middle mb-0">
                ...
                <tbody>
                    <?php if ($data->num_rows == 0): ?>
                        <tr>
                            <td colspan="9" class="text-center text-muted py-5 fw-semibold">
                                <i class="bi bi-inbox fs-2 d-block mb-2"></i> Data tidak ditemukan
                            </td>
                        </tr>
                    <?php endif; ?>
                    ...
                    <?php while ($row = $data->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td data-label="Nama Cabang">
                                <span class="fw-bold" style="color: #1b2559;"><?= h($row['nama_cabang'] ?? '-') ?></span>
                            </td>
